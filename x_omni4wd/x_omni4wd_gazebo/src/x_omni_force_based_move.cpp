...
/*
 * Desc: Gazebo plugin for 4-wheel omni-directional robot with X configuration
 *       Uses a twist message to exert forces on a robot, resulting in motion.
 *       Based on nexus_ros_force_based_move plugin by Stefan Kohlbrecher
 *       
 *       Thứ tự bánh: (1, 2, 3, 4) = (FL, RL, RR, FR)
 *       - Bánh 1: front_left (FL) - góc 45°
 *       - Bánh 2: rear_left (RL) - góc 135°
 *       - Bánh 3: rear_right (RR) - góc -135°
 *       - Bánh 4: front_right (FR) - góc -45°
 * 
 * Author: Adapted for x_omni4wd
 * Date: 2025-01-06
 */

#include <x_omni4wd_gazebo/x_omni_force_based_move.h>

namespace gazebo 
{

  GazeboRosXOmniForceBasedMove::GazeboRosXOmniForceBasedMove() {}

  GazeboRosXOmniForceBasedMove::~GazeboRosXOmniForceBasedMove() {}

  // Load the controller
  void GazeboRosXOmniForceBasedMove::Load(physics::ModelPtr parent,
      sdf::ElementPtr sdf) 
  {
    ...
    // Wheel separation parameters for 4-wheel omni robot
    wheel_separation_x_ = 0.2;  // Default: 0.2m
    if (sdf->HasElement("wheelSeparationX"))
      (sdf->GetElement("wheelSeparationX")->GetValue()->Get(wheel_separation_x_));

    wheel_separation_y_ = 0.2;  // Default: 0.2m
    if (sdf->HasElement("wheelSeparationY"))
      (sdf->GetElement("wheelSeparationY")->GetValue()->Get(wheel_separation_y_));

    ROS_INFO_STREAM("X-Omni4wd ForceBasedMove: wheel_separation_x=" << wheel_separation_x_ 
                    << ", wheel_separation_y=" << wheel_separation_y_ << "\n");

    // Initialize joint pointers for 4 wheels - Thứ tự: (1,2,3,4) = (FL,RL,RR,FR)
    front_left_wheel_joint_ = parent->GetJoint("front_left_wheel_joint");
    rear_left_wheel_joint_ = parent->GetJoint("rear_left_wheel_joint");
    rear_right_wheel_joint_ = parent->GetJoint("rear_right_wheel_joint");
    front_right_wheel_joint_ = parent->GetJoint("front_right_wheel_joint");

    if (!front_left_wheel_joint_ || !rear_left_wheel_joint_ || 
        !rear_right_wheel_joint_ || !front_right_wheel_joint_) {
      ROS_WARN("X-Omni4wd: Some wheel joints not found. Joint state publishing may be disabled.");
    } else {
      ROS_INFO("X-Omni4wd: All 4 wheel joints found successfully.");
    }
    ...
    ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
          boost::bind(&GazeboRosXOmniForceBasedMove::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);
    ...
    joint_state_.name[0] = "front_left_wheel_joint";   // Bánh 1
    joint_state_.name[1] = "rear_left_wheel_joint";    // Bánh 2
    joint_state_.name[2] = "rear_right_wheel_joint";   // Bánh 3
    joint_state_.name[3] = "front_right_wheel_joint";  // Bánh 4

    // start custom queue for omni drive
    callback_queue_thread_ = 
      boost::thread(boost::bind(&GazeboRosXOmniForceBasedMove::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ = 
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&GazeboRosXOmniForceBasedMove::UpdateChild, this));

  }

  // Update the controller
  void GazeboRosXOmniForceBasedMove::UpdateChild()
  {
    ...
    // Apply force for omni-directional movement (x and y directions)
    link_->AddRelativeForce(math::Vector3((std::min(x_, max_x_velocity) - linear_vel.x)* force_x_velocity_p_gain_,
                                          (std::min(y_, max_y_velocity) - linear_vel.y)* force_y_velocity_p_gain_,
                                           0.0));
#endif
  ...
  void GazeboRosXOmniForceBasedMove::FiniChild() {
    alive_ = false;
    queue_.clear();
    queue_.disable();
    rosnode_->shutdown();
  ...
  void GazeboRosXOmniForceBasedMove::cmdVelCallback(
  ...
  void GazeboRosXOmniForceBasedMove::QueueThread()
  {
  ...
  void GazeboRosXOmniForceBasedMove::publishOdometry(double step_time)
  {
  ...
  void GazeboRosXOmniForceBasedMove::publishJointStates()
  {
    ...
    if (front_left_wheel_joint_ && rear_left_wheel_joint_ && 
        rear_right_wheel_joint_ && front_right_wheel_joint_) {
#if (GAZEBO_MAJOR_VERSION >= 8)
      joint_state_.position[0] = front_left_wheel_joint_->Position(0);
      joint_state_.position[1] = rear_left_wheel_joint_->Position(0);
      joint_state_.position[2] = rear_right_wheel_joint_->Position(0);
      joint_state_.position[3] = front_right_wheel_joint_->Position(0);
      
      joint_state_.velocity[0] = front_left_wheel_joint_->GetVelocity(0);
      joint_state_.velocity[1] = rear_left_wheel_joint_->GetVelocity(0);
      joint_state_.velocity[2] = rear_right_wheel_joint_->GetVelocity(0);
      joint_state_.velocity[3] = front_right_wheel_joint_->GetVelocity(0);
#else
      joint_state_.position[0] = front_left_wheel_joint_->GetAngle(0).Radian();
      joint_state_.position[1] = rear_left_wheel_joint_->GetAngle(0).Radian();
      joint_state_.position[2] = rear_right_wheel_joint_->GetAngle(0).Radian();
      joint_state_.position[3] = front_right_wheel_joint_->GetAngle(0).Radian();
      ...
      joint_state_.velocity[3] = front_right_wheel_joint_->GetVelocity(0);
#endif
      joint_state_.effort[0] = 0.0;
      joint_state_.effort[1] = 0.0;
      joint_state_.effort[2] = 0.0;
      joint_state_.effort[3] = 0.0;

      joint_state_pub_.publish(joint_state_);
    }
  }

  void GazeboRosXOmniForceBasedMove::calculateWheelVelocities(double v_x, double v_y, double omega_z,
                                                              double& v_fl, double& v_rl, double& v_rr, double& v_fr)
  {
    // Inverse Kinematics cho 4 bánh omni bố trí chữ X
    // Thứ tự bánh: (1,2,3,4) = (FL,RL,RR,FR)
    // 
    // Công thức:
    ...
    double separation_sum = (wheel_separation_x_ + wheel_separation_y_) / 2.0;
  ...
  tf::Transform GazeboRosXOmniForceBasedMove::getTransformForMotion(double linear_vel_x, double linear_vel_y, double angular_vel, double timeSeconds) const
  {
    tf::Transform tmp;
    tmp.setIdentity();
  ...
  GZ_REGISTER_MODEL_PLUGIN(GazeboRosXOmniForceBasedMove)
}