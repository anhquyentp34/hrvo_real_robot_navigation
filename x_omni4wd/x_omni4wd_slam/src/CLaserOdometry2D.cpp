                            ...
                            const geometry_msgs::Pose& initial_robot_pose)
{
  ...
  ROS_DEBUG("[rf2o] Got first Laser Scan .... Configuring node");
  ...
  angle_min_ = static_cast<float>(scan.angle_min);   // Góc index 0 — dùng đúng convention LaserScan (0 hoặc -π)
...
bool CLaserOdometry2D::odometryCalculation(const sensor_msgs::LaserScan& scan)
{
  ...
  auto m_runtime = ros::Time::now() - start;

  ROS_DEBUG_THROTTLE(1.0, "[rf2o] execution time (ms): %f",
...
void CLaserOdometry2D::createImagePyramid()
{
  ...
  for (unsigned int i = 0; i<pyr_levels; i++)
  {
    ...
    for (unsigned int u = 0; u < cols_i; u++)
    {
      if (range[i](u) > 0.f)
      {
        const float tita = angle_min_ + float(u)*fovh/float(cols_i-1);
...
void CLaserOdometry2D::computeNormals()
{
  ...
  for (unsigned int u=0; u<cols_i; u++)
  {
    if (null(u) == 0.f)
    {
      const float tita = angle_min_ + float(u)*incr_tita;
...
void CLaserOdometry2D::solveSystemOneLevel()
{
  ...
  for (unsigned int u = 1; u < cols_i-1; u++)
    if (null(u) == 0)
    {
      ...
      const float tita = angle_min_ + u/kdtita;
...
void CLaserOdometry2D::performWarping()
{
  ...
  for (unsigned int j = 0; j<cols_i; j++)
  {
    if (range[image_level](j) > 0.f)
    {
      ...
      float tita_w = std::atan2(y_w, x_w);
      const float range_w = std::sqrt(x_w*x_w + y_w*y_w);
      // Chuẩn hóa về [angle_min_, angle_min_+fovh] (lidar có thể 0..2π thay vì -π..π)
      while (tita_w < angle_min_) tita_w += 2.f*static_cast<float>(M_PI);
      while (tita_w > angle_min_ + fovh) tita_w -= 2.f*static_cast<float>(M_PI);
      const float uwarp = kdtita*(tita_w - angle_min_);
  ...
  for (unsigned int u = 0; u<cols_i; u++)
  {
    if (wacu(u) > 0.f)
    {
      const float tita = angle_min_ + float(u)/kdtita;
...
bool CLaserOdometry2D::filterLevelSolution()
{
  ...
  {
    ROS_WARN( "[rf2o] ERROR: Eigensolver couldn't find a solution. Pose is not updated");
    return false;
  }
...
void CLaserOdometry2D::PoseUpdate()
{
  ...
  ROS_DEBUG_THROTTLE(1.0, "[rf2o] LASERodom = [%f %f %f]",
  ...
  ROS_DEBUG_THROTTLE(1.0, "BASEodom = [%f %f %f]",
