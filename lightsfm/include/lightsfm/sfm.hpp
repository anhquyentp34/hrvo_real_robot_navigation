...
struct Parameters {
  Parameters()
    : forceFactorDesired(2.0), forceFactorObstacle(10),
      ...
      forceFactorGroupRepulsion(1.0), lambda(2.0), gamma(0.35),
      n(2.0), epsilon(0.005), nPrime(3.0), relaxationTime(0.5),
...
SocialForceModel::computeSocialForce(unsigned index,
  ...
  for (unsigned i = 0; i < agents.size(); i++) {
	...
	if (dist > agent.params.interactionRange) {
	    continue;   // bỏ qua người quá xa
	}
...
SocialForceModel::computeSocialForce(Agent &me,
  ...
  for (unsigned i = 0; i < agents.size(); i++) {
    ...
    double dist = diff.norm();
    if (dist > me.params.interactionRange) {
	continue;
    }
