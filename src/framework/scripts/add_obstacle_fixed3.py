...
def predicted_collision_point_callback(msg, robot_name):
    ...
    obstacle_publisher[robot_name].publish(obstacle_array)
    ...
    marker_publisher[robot_name].publish(cylinder_marker)

rospy.init_node("circle_obstacle_publisher")

robot_names = ["actor1", "actor2"]
obstacle_publisher = {}
marker_publisher = {}

for robot_name in robot_names:
    obstacle_publisher[robot_name] = rospy.Publisher(f"/{robot_name}/move_base/TebLocalPlannerROS/obstacles", ObstacleArrayMsg, queue_size=1)
    marker_publisher[robot_name] = rospy.Publisher(f"/{robot_name}/cylinder_marker", Marker, queue_size=1)

    # Subscribe to the predicted_collision_point topic for each robot
    rospy.Subscriber(f"/{robot_name}/predicted_collision_point", PointStamped, predicted_collision_point_callback, callback_args=robot_name, queue_size=1)
