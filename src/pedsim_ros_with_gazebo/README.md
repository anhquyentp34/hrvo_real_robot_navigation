...
* Integrate Pedsim_ros with the 3D gazebo simulator and a Kobuki-style stack (`robot_gazebo`);
...
## Installation
The default version is ROS Noetic. This workspace includes `robot_gazebo`, `yocs_cmd_vel_mux`, and `kobuki_gazebo_plugins` under `src/`. Use APT for `depthimage_to_laserscan` (ROS 1 — do not vendor the ROS 2 branch).

Install binary deps on Ubuntu, then build:
```
sudo apt install ros-noetic-depth-image-to-laserscan ros-noetic-teleop-twist-keyboard ros-noetic-kobuki-msgs
cd ~/catkin_ws   # or your workspace root
catkin_make
...
```
RViz mesh người đi bộ (cùng ý tưởng ICR2026 + `animated_markers`): `roslaunch pedsim_simulator robot.launch enable_actor_animated_markers:=true` — trong RViz thêm display `AnimatedMarkerArray` (`animated_marker_rviz_plugin`), topic `/actor_visualization`.

Chỉ người + Gazebo, không spawn robot Kobuki/Turtle: `with_robot:=false`. Trên Ubuntu chỉ có `python3`: các script Pedsim đã dùng shebang `python3` (tránh lỗi spawn agent).
