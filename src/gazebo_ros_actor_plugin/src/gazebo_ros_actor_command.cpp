...
void GazeboRosActorCommand::Reset() {
  ...
  this->last_update_ = 0;
  this->idx_ = 0;
  // Clear target poses vector - don't initialize with origin
  this->target_poses_.clear();
  // Don't set target pose until we receive path commands
...
void GazeboRosActorCommand::PathCallback(const nav_msgs::Path::ConstPtr &msg) {
  ...
  // Set the first target pose if we have any poses
...
void GazeboRosActorCommand::OnUpdate(const common::UpdateInfo &_info) {
  ...
  if (this->follow_mode_ == "path") {
    // Check if we have any target poses
    if (this->target_poses_.empty()) {
      // No targets yet, don't move
      this->last_update_ = _info.simTime;
      return;
    }
    
    // Ensure we have a valid target index
    if (this->idx_ >= this->target_poses_.size()) {
      this->idx_ = 0;
    }
